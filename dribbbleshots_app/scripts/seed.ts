...
async function seed() {
    ...
    await db.run(sql`DELETE FROM ai_evaluations;`);
    await db.run(sql`DELETE FROM admin_reviews;`);
    await db.run(sql`DELETE FROM notifications;`);
    await db.run(sql`DELETE FROM shot_plans;`);
    await db.run(sql`DELETE FROM theme_library;`);

    // reset auto increment for clean re-seed
    await db.run(sql`UPDATE sqlite_sequence SET seq = 0 WHERE name = 'theme_library';`);

    console.log('Inserting Theme Library...');
    for (const theme of seedThemes) {
        await db.insert(themeLibrary).values(theme).onConflictDoNothing();
    }
